...
BEGIN
    ...
    ;WITH commonTypes AS
    (
        SELECT DISTINCT preType.ValidationType
        FROM (SELECT DISTINCT ValidationType FROM dbo.ValidationResult WHERE RunId = @PreRunId) preType
        INNER JOIN (SELECT DISTINCT ValidationType FROM dbo.ValidationResult WHERE RunId = @PostRunId) postType
            ON postType.ValidationType = preType.ValidationType
    )
    ...
    (
     ...
     WHERE @CompareCommonTypesOnly = 0
         OR COALESCE(postr.ValidationType, prer.ValidationType) IN (SELECT ValidationType FROM commonTypes);
END;
GO
...
CREATE OR ALTER PROCEDURE dbo.usp_ValidationRun_Purge
    @RetentionDays int
AS
BEGIN
    SET NOCOUNT ON;
    ...
    BEGIN
        THROW 51001, 'RetentionDays must be greater than 0.', 1;
    END;

    DECLARE @Cutoff datetime2(0) = DATEADD(DAY, -@RetentionDays, sysdatetime());

    DELETE vr
    FROM dbo.ValidationResult vr
    INNER JOIN dbo.ValidationRun r
        ON r.RunId = vr.RunId
    WHERE r.StartTime < @Cutoff;

    DELETE dbo.ValidationRun
