...
function Test-SqlInstance {
    ...
    try {
        ...
        if (Test-InvokeSqlcmdSupportsTrustServerCertificate) {
            $invokeParams.TrustServerCertificate = $true
        }

        $row = Invoke-Sqlcmd @invokeParams
...
if (Test-InvokeSqlcmdSupportsTrustServerCertificate) {
    Write-Host 'Invoke-Sqlcmd supports TrustServerCertificate.' -ForegroundColor Green
}
else {
    Write-Host 'Invoke-Sqlcmd does not support TrustServerCertificate. Preflight will test connectivity without it.' -ForegroundColor Yellow
}
...
if ($RunFrameworkSmoke) {
    ...
    $settingsPath = Join-Path -Path $FrameworkRoot -ChildPath 'settings.production.patch.json'
    $runtimeSettingsPath = Join-Path -Path ([System.IO.Path]::GetTempPath()) -ChildPath ("PatchValidation.Smoke.{0}.json" -f ([guid]::NewGuid().ToString('N')))
    $scriptPath = Join-Path -Path $FrameworkRoot -ChildPath 'Scripts\Invoke-Validation.ps1'
    if (-not (Test-Path -Path $scriptPath -PathType Leaf)) {
        $scriptPath = Join-Path -Path $FrameworkRoot -ChildPath 'Invoke-Validation.ps1'
    }

    if (-not (Test-Path -Path $settingsPath -PathType Leaf)) {
        throw "settings.production.patch.json not found: $settingsPath"
    }
    ...
    $settings | ConvertTo-Json -Depth 10 | Set-Content -Path $runtimeSettingsPath -Encoding utf8

    PowerShell.exe -NoProfile -ExecutionPolicy Bypass -File $scriptPath -Stage PRE -PatchBatchId $PatchBatchId -ConfigPath $runtimeSettingsPath
}
