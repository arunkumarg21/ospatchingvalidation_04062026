...
if ($PSBoundParameters.ContainsKey('RunId')) {
    ...
    if (-not [string]::IsNullOrWhiteSpace($runIdText)) {
        $parsedRunId = 0
        if (-not [int]::TryParse($runIdText, [ref]$parsedRunId)) {
            throw "Invalid -RunId value '$runIdText'. RunId must be an integer."
        }

        $normalizedRunId = $parsedRunId
    }
}

$RunId = $normalizedRunId

# Normalize validation type arguments. This handles direct arrays and comma-delimited forwarding.
if ($ValidationTypes) {
    $ValidationTypes = @(
        foreach ($validationTypeValue in $ValidationTypes) {
            if ($null -eq $validationTypeValue) {
                continue
            }

            foreach ($token in ([string]$validationTypeValue -split ',')) {
...
$moduleRoot = $moduleCandidates | Where-Object {
    Test-Path -Path (Join-Path -Path $_ -ChildPath 'Common.psm1') -PathType Leaf
} | Select-Object -First 1

if (-not $moduleRoot) {
    throw 'Could not locate Common.psm1 and Logging.psm1. Expected in ..\Modules, script folder, or parent folder.'
}
...
function Invoke-ValidationChildProcess {
    ...
    $arguments = @(
        ...
        '-ValidationTypes', $CurrentValidationType
        '-Worker'
...
function Invoke-SQLConnectivityValidation {
    param([int]$CurrentRunId, [string]$ServerName, $Config)
    ...
    foreach ($row in $rows) {
        Write-ValidationResult -Config $Config -RunId $CurrentRunId -ServerName $ServerName `
            -ValidationType 'SQLConnectivity' -ResultName 'ConnectToMaster' -ResultKey 'SQLConnectivity|ConnectToMaster' `
            -ExpectedValue 'SQL connection succeeds' -ActualValue ('Connected to {0}; Version={1}' -f $row.ServerName, $row.ProductVersion) -ValidationStatus 'PASS' `
            -Details @{ ServerName = $row.ServerName; ProductVersion = $row.ProductVersion }
    }
}
...
function Invoke-SQLServicesValidation {
    ...
    $services = Get-CimInstance -ClassName Win32_Service -ComputerName $ServerName -Filter "Name LIKE 'SQL%' OR Name LIKE '%SQL%'" -ErrorAction Stop |
...
BEGIN
    SELECT
        ...
        primary_server,
        primary_database,
        ISNULL(CONVERT(nvarchar(30), last_copied_date, 126), last_copied_file) AS LastCopiedSignal,
        ISNULL(CONVERT(nvarchar(30), last_restored_date, 126), last_restored_file) AS LastRestoredSignal,
        CASE WHEN secondary_id IS NULL THEN 'UNKNOWN' ELSE 'CONFIGURED' END AS SecondaryStatus
...
"@
    ...
    foreach ($row in $rows) {
        ...
        Write-ValidationResult -Config $Config -RunId $CurrentRunId -ServerName $ServerName `
            ...
            -ExpectedValue 'No POST deviation from PRE' -ActualValue ('LastRestoredSignal={0}' -f $row.LastRestoredSignal) -ValidationStatus 'INFO' `
            -Details $row
    }
}
...
function Invoke-WindowsClusterStatusValidation {
    ...
    $clusterService = Get-CimInstance -ClassName Win32_Service -ComputerName $ServerName -Filter "Name='ClusSvc'" -ErrorAction Stop
...
function Invoke-WindowsServicesValidation {
    ...
    $services = @(Get-CimInstance -ClassName Win32_Service -ComputerName $ServerName -Filter $filter -ErrorAction Stop)
    $serviceMap = @{}
    foreach ($service in $services) {
...
function Invoke-WindowsEventLogValidation {
    ...
    foreach ($eventRecord in $events) {
        $key = 'WindowsEventLog|{0}|{1:yyyyMMddHHmmss}' -f $eventRecord.Id, $eventRecord.TimeCreated
        Write-ValidationResult -Config $Config -RunId $CurrentRunId -ServerName $ServerName `
            -ValidationType 'WindowsEventLog' -ResultName $eventRecord.ProviderName -ResultKey $key `
            -ExpectedValue 'No Critical/Error events' -ActualValue $eventRecord.LevelDisplayName -ValidationStatus 'WARN' `
            -Details $eventRecord
    }
}
...
function Invoke-WindowsPatchHistoryValidation {
    ...
    $patches = Get-CimInstance -ClassName Win32_QuickFixEngineering -ComputerName $ServerName -ErrorAction Stop |
...
try {
    ...
    if (-not $RunId -or $RunId -lt 1) {
        throw "Unable to create a valid RunId for PatchBatchId '$PatchBatchId' and Stage '$Stage'."
    }
            ...
            try {
                ...
                Invoke-WithRetry -RetryCount $config.RetryCount -RetryDelaySeconds $config.RetryDelaySeconds -OperationName "$validationType on $server" -ScriptBlock {
                    switch ($validationType) {
                        ...
                        'SQLConnectivity' { Invoke-SQLConnectivityValidation -CurrentRunId $RunId -ServerName $server -Config $config }
    ...
    if (-not $Worker) {
        $errorCount = [int](Invoke-RepositoryScalar `
            -ServerInstance $config.RepositoryServer `
            -DatabaseName $config.RepositoryDatabase `
            -CommandTimeoutSeconds $config.CommandTimeoutSeconds `
            -CommandText "SELECT COUNT(1) FROM dbo.ValidationResult WHERE RunId = @RunId AND ValidationStatus = 'ERROR';" `
            -Parameters @{ '@RunId' = $RunId })

        $warnCount = [int](Invoke-RepositoryScalar `
            -ServerInstance $config.RepositoryServer `
            -DatabaseName $config.RepositoryDatabase `
            -CommandTimeoutSeconds $config.CommandTimeoutSeconds `
            -CommandText "SELECT COUNT(1) FROM dbo.ValidationResult WHERE RunId = @RunId AND ValidationStatus IN ('WARN','FAIL');" `
            -Parameters @{ '@RunId' = $RunId })

        $finalStatus = if ($errorCount -gt 0 -or $warnCount -gt 0) { 'COMPLETED_WITH_WARNINGS' } else { 'SUCCESS' }
        $finalMessage = "Validation completed. Errors=$errorCount Warnings=$warnCount LogFile=$logFile"

        Complete-ValidationRun -Config $config -RunId $RunId -Status $finalStatus -Message $finalMessage
        Write-ValidationLog -Message "SQL patch validation completed. RunId=$RunId Status=$finalStatus Errors=$errorCount Warnings=$warnCount"
        Write-Output $RunId
    }
}
catch {
